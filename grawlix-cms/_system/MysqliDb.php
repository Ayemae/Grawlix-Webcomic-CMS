...
class MysqliDb
{
	...
	/**
	 * Set the timezone based on a timezone string.
	 * We need this to make sure dates and times in queries are correct,
	 * but we can't do it at connection-time because the timezone data
	 * may be stored in the DB.
	 * 
	 * @param string $timezone     Contains a timezone string.
	 */
	public function setTimezone($timezone) {
		...
		$timezoneRound = round($timezone, 0, PHP_ROUND_HALF_DOWN);
		if($timezone == $timezoneRound) {
			$timezone = str_pad($timezone, 2, '0', STR_PAD_LEFT);
			$this->_mysqli->query('SET @@session.time_zone = "'.$sign.$timezone.':00"');
		} else {
			$minutes = abs($timezone - $timezoneRound) * 60;
			$minutes = floor($minutes);
			$timezoneRound = str_pad($timezoneRound, 2, '0', STR_PAD_LEFT);
			$this->_mysqli->query('SET @@session.time_zone = "'.$sign.$timezoneRound.":".$minutes.'"');
		}
	}
