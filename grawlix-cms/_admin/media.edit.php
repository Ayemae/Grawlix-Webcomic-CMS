...
if ( !empty($_POST) && !empty($description) ) {
...
if ( !empty($_FILES) ) {
	...
	if ( !empty($_FILES['file_change']) && !empty($_FILES['file_change']['name']) ) {
...
if (isset( $action) && $action == 'gen-thumb') {
	$db->where('id',$image_id);
	$image_info = $db->getOne('image_reference','id,description,url,date_created,date_modified');
...
if ( !empty($image_dimensions) && !empty($image_bytes) ) {
...
if ( empty($image_dimensions) || count($image_dimensions) == 0 ) {
