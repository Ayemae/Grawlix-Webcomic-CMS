...
$success1 = false;
if (!empty($_FILES)) {
	if (isset($_FILES['file']) && $_FILES['file']['type'][0] != 'text/xml') {
...
if ( isset($comic_page_list) && isset($image_list) ) {
...
// ! Create new markers
$completed = false;
if ( count($chapter_list) > 0 && $user_confirmed == 1 ) {
	// ! Make a new book, if necessary
	if (isset($new_book_name) && $new_book_name != '') {
...
if ( isset($comic_page_list) && $completed === FALSE) {
...
if ( empty($comic_page_list) && $completed === FALSE) {
